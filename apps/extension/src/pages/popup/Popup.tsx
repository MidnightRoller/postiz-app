...
export const PopupContainerContainer: FC = () => {
  ...
  if (!url) {
    return (
      <div className="text-4xl">This website is not supported by </div>
    );
  }
...
export const PopupContainer: FC<{ url: string }> = (props) => {
  ...
  if (!provider) {
    return (
      <div className="text-4xl">This website is not supported by Social</div>
    );
  }

  if (!isLoggedIn) {
    return <div className="text-4xl">You are not logged in to Social</div>;
  }
